...
encrypting messages, SHA hashes an arbitrary size message into a fixed-size irreversible hash.
...
We then use it to accelerate Unix password hashing and implement
...
each block is then processed sequentially using the
...
Unix password cracker. Most unix systems store the passwords hashes in the
``/etc/shadow'' file. Entries in this file have the format: ``[user]:\$[hash\_algorithm]\$salt\$hash''.
...
and expensive operation to compute. When testing through the $crypt()$ function
...
synthesizable. Ultimately, we ended up with \emph{SHA512Hasher}, a C++ class that exposed the following methods:
\begin{verbatim}
...
\emph{update()} is repeatedly called on it. Whenever the buffer fills up, the class
automatically (in the \emph{update()} call) runs the SHA-512 algorithm for the buffered block
...
other projects. This generality was in fact needed later to support variable length passwords with the Unix password
...
optimizing this \emph{hashBlock()} method was the first priority. Initially, its
latency was $534$ cycles. However, a number of optimizations were applied that
eventually brought the latency down to $88$ cycles.

One of the most important optimizations
involved recognizing that the $i^{th}$ value of $W$ (\ref{SHA512}) only relies on the 15 preceding $W$ values.
...
be initialized at the beginning of \emph{hashBlock()} as well as updated
...
\emph{SHA512Hasher} methods, so resources were conserved and instead used to enable the
\emph{SHA512Hasher} optimizations explained previously.

A basic host program was written, \emph{zedboard/host.cpp}, which sends a
...
version are nearly identical to the software implementations, but they make use of optimization
...
\caption {Results for the Unix Password Hasher. The ecelinux and zedboard versions were idential software
  ...
  information was taken from the manufacturers' websites. The Intel CPU has a TDP of 120W, and Xilinix
...
improvement over the Zedboard CPU. Even though the Intel CPU predictably beats the FPGA in terms of raw
...
has a ratio of 0.524, while the FPGA achieves .00200, \textit{over two orders of magnitude lower} than the
