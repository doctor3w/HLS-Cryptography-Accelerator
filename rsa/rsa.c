...
#define MODULUS_SIZE \
  1024 /* This is the number of bits we want in the modulus */
#define BLOCK_SIZE \
  (MODULUS_SIZE /  \
   8) /* This is the size of a block that gets en/decrypted at once */
#define BUFFER_SIZE \
  ((MODULUS_SIZE / 8) / 2) /* This is the number of bytes in n and p */
...
int encrypt(char cipher[], char message[], int length, public_key_t kp) {
  ...
  while (prog > 0) {
    ...
    off += (BLOCK_SIZE -
            (mpz_sizeinbase(c, 2) + 8 - 1) / 8);  // See manual for mpz_export
...
int decrypt(char* message, char* cipher, int length, private_key_t ku) {
  ...
  *(long long*)buf = 0ll;
  ...
  for (i = 0; i < (length / BLOCK_SIZE); i++) {
    ...
    // pad with zeros in the front if the number we get back has fewer bits
    // than BLOCK_SIZE
    int off = (BLOCK_SIZE - (mpz_sizeinbase(m, 2) + 8 - 1) /
                                8);  // See manual for mpz_export
